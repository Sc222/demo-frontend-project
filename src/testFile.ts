...
export class Test implements ITest {
  ...
  public static StaticMethod() {
    return this.StaticVariable === 1 ? "OK" : this.StaticVariable === 231231 ? "ERR" : this.StaticVariable === 345345 ? "PENDING" : this.StaticVariable === 4444 ? "FINISHED" : "UNKNOWN_CODE";
  }

  public publicMethod() {
    for (let loops = 0; loops < 2; loops++)
      for (let i = 0; i < this.publicVariable.length; i++)
        console.log(this.publicVariable[i]);

    if (this.publicVariable.length === 0)
      return;
